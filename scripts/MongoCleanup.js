...
function main() {
    // true to avoid actually modifying db
    DRY_RUN = true;
    ...
    // Level 1
    // 59e90b3c4f0c079c55c2cbfc - RGB GeoTIFFs

    print("NAME,TYPE,STATUS,ID,OWNER,PARENT_TARGET,PARENT_ACTUAL,NOTES");

    //cleanFiles(); CURRENTLY SKIPPED DUE TO SLOWNESS
    cleanCollections();
    cleanDatasets();
    healMisnamed();
    healOrphans();

    // TODO: Fix for mis-elevated collections, not sure if needed
    // db.collections.update({'root_spaces.0': {$exists: true}, 'parent_collection_ids.0': {$exists: true}},
    //                       {$set: {"root_spaces": []}}, {"multi":true});
}
...
function cleanFiles() {
    ...
    while (duplicates.hasNext()) {
        ...
        while (subs.hasNext()) {
            ...
            if (!DRY_RUN) {
                db.uploads.remove({"_id": subdoc._id});
            }
        }
    }
}
...
function cleanCollections() {
    ...
    while (duplicates.hasNext()) {
        ...
        bigboss = db.collections.findOne({
            "name": dupe,
            "$or": OFFICIAL_OWNERS,
            "parent_collection_ids": {$size: 1}
        });
        ...
        //meta_count = bigboss.metadataCount (UNUSED)

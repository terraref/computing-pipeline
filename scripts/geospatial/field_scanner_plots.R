...
for (i in (1:(ranges/2)) * 2) {
  grid[((i - 1) * columns + 1):(i * columns), 1] <- c((i * columns):((i - 1) * 
    columns + 1))
}
...
gantry2latlon <- function(Gx, Gy) {
  ...
  # MAC --> USDA
  result[, 1] <- result[, 1] + 2.0308287e-05
  result[, 2] <- result[, 2] - 1.5258894e-05
  return(result)
}
...
latlon2length <- function(lon1, lat1, lon2, lat2) {
  ...
  length <- sqrt((UTM_pt1[, 1] - UTM_pt2[, 1])^2 + (UTM_pt1[, 2] - UTM_pt2[, 
    2])^2)
  return(length)
}
...
for (i in 1:(ranges * columns)) {
  # tablename = 'yourtablename' sql = paste('INSERT INTO', tablename,
  # '(plotid,range,col,geomgantry,x,y,gantryw,gantryl,geomusda,lon,lat,utmw,utml)')
  # temp=paste0(grid$pt1X[i],' ',grid$pt1Y[i],', ', grid$pt2X[i],'
  # ',grid$pt2Y[i],', ', grid$pt3X[i],' ',grid$pt3Y[i],', ', grid$pt4X[i],'
  # ',grid$pt4Y[i],', ', grid$pt1X[i],' ',grid$pt1Y[i])
  # geom_gantry=paste0('ST_GeomFromText('POLYGON((',temp,'))'',')')
  
  temp <- paste0(grid$lon1[i], " ", grid$lat1[i], " 353, ", grid$lon2[i], " ", 
    grid$lat2[i], " 353, ", grid$lon3[i], " ", grid$lat3[i], " 353, ", grid$lon4[i], 
    " ", grid$lat4[i], " 353, ", grid$lon1[i], " ", grid$lat1[i], " 353")
  geom_latlon <- paste0("ST_GeomFromText('POLYGON((", temp, "))'", ", 4326)")
  
  # sql = paste(sql,'VALUES (',paste(grid$plot[i],grid$range[i],
  # grid$column[i],geom_gantry,grid$pt1X[i],grid$pt1Y[i],
  # grid$plot_W[i],grid$plot_L[i],geom_latlon,grid$lon1[i],grid$lat1[i],
  # grid$UTM_W[i],grid$UTM_L[i],sep=', '),');')
  
  sql <- paste0("INSERT INTO sites (sitename, geometry) VALUES ( ", paste0("'MAC Field Scanner Field Plot ", 
    grid$plot[i], " Season 2', "), geom_latlon, ");")

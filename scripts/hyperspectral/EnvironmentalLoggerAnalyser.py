...
3. numpy (numpy is required for the matrix multiplication)
----------------------------------------------------------------------------------------
...
flx_dwn = flx_sns (dot_product) spectrum

***** Recently the way to do it is by dot producting the flx_sns(dimesion: 1*1024) with 
each element in spectrum (dimension: 1*1024, 39 total for each JSON file so far).
Each time the calculation above will produce a scalar result, and since there are 39 
readings per JSON, the final result would also be an array with a dimension of 39. *****
----------------------------------------------------------------------------------------
...
def wavelengthSpectrumAndDownwellingFlux(fileLocation):
    ...
    with open(fileLocation, 'r') as fileHandler:
        ...
        spectrumList.remove([])
        downwellingFlux = [(numpy.dot(_FLX_SNS, data) / 1e-6)
                           * _WVL_DLT for data in spectrumList]
...
def JSONHandler(fileLocation):
    ...
    '''
    wavelength, spectrum, downwellingFlux = wavelengthSpectrumAndDownwellingFlux(
        fileLocation)
    with open(fileLocation, 'r') as fileHandler:
        return json.loads(fileHandler.read()), wavelength, spectrum, downwellingFlux
...
def main(JSONArray, outputFileName, wavelength=None, spectrum=None, downwellingFlux=None, recordTime=None, commandLine=None):
    '''
    ...
    netCDFHandler.createVariable("wvl_dlt", 'f8').assignValue(0.4e-9)
    setattr(netCDFHandler.variables['wvl_dlt'], 'units', 'meter')
    setattr(netCDFHandler.variables['wvl_dlt'], 'notes', "Best information available is that bandwidth\
    is uniform 0.4 nm across all channels")
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(fileInputLocation) or fileOutputLocation.endswith('.nc'):
        ...
        tempJSONMasterList, wavelength, spectrum, downwellingFlux = JSONHandler(
            fileInputLocation)
        if not os.path.isdir(fileOutputLocation):
            main(tempJSONMasterList, fileOutputLocation, wavelength, spectrum, downwellingFlux,
            ...
            for members in fileName:
                if os.path.join(filePath, members).endswith('.json'):
                    ...
                    main(tempJSONMasterList, os.path.join(
                        ...
                        wavelength, spectrum, downwellingFlux, _timeStamp(), sys.argv[1] + ' ' + sys.argv[2])

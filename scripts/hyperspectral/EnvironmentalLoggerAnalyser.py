...
def main(JSONArray, outputFileName):
	...
	for data in dataMemberList[0]: 
		tempGroup = netCDFHandler.createGroup(renameTheValue(data))
		if 'unit' in dataMemberList[0][data]: #Assign Units
			tempVariable = tempGroup.createVariable('unit', str)
			tempVariable[0] = _UNIT_DICTIONARY[dataMemberList[0][data]['unit']]
			setattr(tempGroup, 'unit', _UNIT_DICTIONARY[dataMemberList[0][data]['unit']])
		if 'value' in dataMemberList[0][data]: #Assign "values"
			tempGroup.createVariable('value', 'f8', ('time',))[:] = getListOfValue(dataMemberList, data)
		if 'rawValue' in dataMemberList[0][data]: #Assign "rawValues"
			tempGroup.createVariable('rawValue', 'f8', ('time',))[:] = getListOfValue(dataMemberList, data)
...
if __name__ == '__main__':
	...
	if not os.path.isdir(fileInputLocation):
		...
		for filePath, fileDirectory, fileName in os.walk(fileInputLocation): #Read and Export netCDF to folder
